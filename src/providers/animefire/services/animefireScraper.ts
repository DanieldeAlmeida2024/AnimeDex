...
import { ScrapedAnime, ScrapedEpisode, ScrapedStream } from '../../../utils/types/types';
...
                const fullAnimefireUrl = animefireUrl.startsWith('http') ? animefireUrl : `${BASE_URL}${animefireUrl}`;
                ...
                const inferredType: 'movie' | 'series' = (normalizedTitle.includes('film') || normalizedTitle.includes('movie')) ? 'movie' : 'series';
...
export async function scrapeTopAnimes(type: 'movie' | 'series', page: number = 1): Promise<ScrapedAnime[]> {
...
export async function scrapeAtualizadosAnimes(type: 'movie' | 'series', page: number = 1): Promise<ScrapedAnime[]> {
...
export async function scrapeDubladosAnimes(type: 'movie' | 'series', page: number = 1): Promise<ScrapedAnime[]> {
...
export async function scrapeLegendadosAnimes(type: 'movie' | 'series', page: number = 1): Promise<ScrapedAnime[]> {
...
export async function searchAnimes(query: string, requestedType?: 'movie' | 'series'): Promise<ScrapedAnime[]> {
    const formattedQueryForUrl = query.trim().replace(/\s+/g, '-').toLowerCase();
    const url = `${BASE_URL}${SEARCH_URL}/${formattedQueryForUrl}`;
    const animes: ScrapedAnime[] = [];
